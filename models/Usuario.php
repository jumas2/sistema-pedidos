...
class Usuario {
    ...
    public function getClientes($usuario_id) {
        $stmt = $this->db->prepare("
            SELECT c.* 
            FROM clientes c
            JOIN usuario_cliente uc ON c.id = uc.cliente_id
    ...
    public function getClientesDisponibles($usuario_id) {
        $stmt = $this->db->prepare("
            SELECT c.* 
            FROM clientes c
            WHERE c.id NOT IN (
                SELECT cliente_id FROM usuario_cliente WHERE usuario_id = :usuario_id
            )
            ORDER BY c.nombre
        ");
        $stmt->execute(['usuario_id' => $usuario_id]);
    ...
    public function asignarCliente($usuario_id, $cliente_id) {
        $stmt = $this->db->prepare("
            SELECT id FROM usuario_cliente 
            WHERE usuario_id = :usuario_id AND cliente_id = :cliente_id
        ");
        $stmt->execute(['usuario_id' => $usuario_id, 'cliente_id' => $cliente_id]);
        if ($stmt->fetch()) {
            return false;
        }
        
        $stmt = $this->db->prepare("
            INSERT INTO usuario_cliente (usuario_id, cliente_id) 
            VALUES (:usuario_id, :cliente_id)
        ");
        return $stmt->execute(['usuario_id' => $usuario_id, 'cliente_id' => $cliente_id]);
    }
    
    public function eliminarCliente($usuario_id, $cliente_id) {
        $stmt = $this->db->prepare("
